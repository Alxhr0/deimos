...
build $target_image=image_name $tag=default_tag:
    ...
    if [[ -z "$(git status -s)" ]]; then
        ...
        LABELS+=("--label" "org.opencontainers.image.source=https://github.com/{{ repo_organization }}/{{ image_name }}/blob/${GIT_SHA}/images/{{ image_name }}/Containerfile")
    ...
    PODMAN_BUILD_ARGS=("${BUILD_ARGS[@]}" "${LABELS[@]}" --pull=newer --tag "${target_image}:${tag}" --file images/{{ image_name }}/Containerfile)
